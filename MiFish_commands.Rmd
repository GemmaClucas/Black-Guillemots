...
All analyses performed in the folder `Fecal_metabarcoding/Black-Guillemots`. We have four plates on which the samples have been sequenced: 24, 64, 81, and 146.
```
conda activate qiime2-amplicon-2024.10 
mkdir MiFish
cd MiFish
```
Will have to redo everything once we have redone plate 146 with the MiFish primers.
...
```
Plate 146 looks very different to the other samples as fish are consistently being assigned to different taxa because of the use of the wrong primers.

MiniFish primers have missed Atlantic herring in the mock community, but there is no herring in the samples from any of the other years, so this is hopefully not a problem. Are they missing other things though?
...
```
The extraction-, field-, and PCR-banks all have barely any reads in them.
...
```
Species richness plateaus in the samples at 200 reads, so just drop samples with fewer than 200. This drops the final extraction blank, two of the field blanks, and three samples. Two field blanks remain with 312 and 1,102 reads.
...
* Atlantic wolffish changed to wolffish sp. although the Atlantic wolffish is by far the most common in the GoM out of the three possibilities according to Bigelow and Schroeder.   
* Three sculpins (Myoxocephalus sp.) are possible in the GoM - all common and genetically alike: longhorn, shorthorn, and grubby.   
* There is only one wrymouth found in the GoM according to Bigelow and Schroeder.  
* There are four Liparis spp in the GoM, but none were on genbank.  
* The redfish (Sebastes) species is most likely acadian redfish (S. fasciatus).   
...
I have modified this from the code I had for the UK gannet project. I had to remove the 1% abundance filtering and changed it to keep the sample metadata in the output.
```{r}
library(qiime2R)
